...
try:
    ...
    async def process_trade_messages(trades):
        ...
        try:
            ...
            for trade in trades:
                try:
                    # Extract trader addresses from trade data - FIXED to use the 'users' array
                    if "users" in trade and isinstance(trade["users"], list):
                        for trader_address in trade["users"]:
    ...
    async def test_websocket_discovery(duration=20):
    ...
    if __name__ == "__main__":
        # Get test duration from command line argument, default to 20 seconds
